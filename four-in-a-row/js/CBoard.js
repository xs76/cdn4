function CBoard(iX,iY,oParentContainer){
    ...
    this._init = function(iXPos,iYPos){
        ...
        for(var k=0;k<BOARD_COLS;k++){
            ...
            _aListeners.push( {listener:oListener,event:"click"});
            if(!s_bMobile){
                oHitArea.cursor = "pointer";
                oListener = oHitArea.on("mouseover",_oThis.colOver,null,false,k);
                _aListeners.push( {listener:oListener,event:"onmouseover"});
            }
            ...
            _aStartPosForDiscs[k] = {x:_aBoardCellPos[_aBoardCellPos.length-1][k].x,y:_aBoardCellPos[_aBoardCellPos.length-1][k].y - DISC_DIAMETER};
        }
        ...
        _aStartOffsetCheckDiagDx.push( { row : 2, col : 0 } );
        _aStartOffsetCheckDiagDx.push( { row : 1, col : 0 } );
        _aStartOffsetCheckDiagDx.push( { row : 0, col : 0 } );
        _aStartOffsetCheckDiagDx.push( { row : 0, col : 1 } );
        _aStartOffsetCheckDiagDx.push( { row : 0, col : 2 } );
        _aStartOffsetCheckDiagDx.push( { row : 0, col : 3 } );
        
        _aStartOffsetCheckDiagSx = new Array();	
        _aStartOffsetCheckDiagSx.push( { row : 2, col : 6 } );
        _aStartOffsetCheckDiagSx.push( { row : 1, col : 6 } );
        _aStartOffsetCheckDiagSx.push( { row : 0, col : 6 } );
        _aStartOffsetCheckDiagSx.push( { row : 0, col : 5 } );
        _aStartOffsetCheckDiagSx.push( { row : 0, col : 4 } );
        _aStartOffsetCheckDiagSx.push( { row : 0, col : 3 } );	
    };
    ...
    this.colOver = function(evt,iCol){
        ...
        createjs.Tween.get(_oArrow).to({x: _aStartPosForDiscs[iCol].x}, 500,createjs.Ease.cubicOut);
    };
    ...
    this.colSelected = function(evt,iCol){
        ...
        var oDisc = new CDisc({x:_aStartPosForDiscs[iCol].x,y:_aStartPosForDiscs[iCol].y},s_oGame.getCurTurn(),_oDiscContainer);
        oDisc.drop(iTime,{x:_aBoardCellPos[iRow][iCol].x,y:_aBoardCellPos[iRow][iCol].y});
    ...
    this.getPointPerCol = function(iCol){
        ...
        var pStart = {x:_aStartPosForDiscs[iCol].x,y:_aStartPosForDiscs[iCol].y};
        var pEnd = {x:_aBoardCellPos[iRow][iCol].x,y:_aBoardCellPos[iRow][iCol].y};
        
        return {start:pStart,end:pEnd,row:iRow};
    };

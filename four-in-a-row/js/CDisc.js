function CDisc(pStartPos,iType,oParentContainer){
    ...
    this._init = function(pStartPos,iType){
        ...
        var oData = {   
                        images: [oSprite], 
                        // width, height & registration point of each sprite
                        frames: {width: 78, height: 78, regX: 39, regY: 39}, 
                        animations: {stop:[0,0],anim:[0,27]}
                   };
    ...
    this.drop = function(iTime,pEndPos){
        ...
        playSound("drop",1,0);
        createjs.Tween.get(_oContainer).to({y: pEndPos.y}, iTime,createjs.Ease.bounceOut).call(function(){s_oGame.checkCombos()});
    };

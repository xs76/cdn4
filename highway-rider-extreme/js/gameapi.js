...
fg_api.prototype.navigationModule = function() {
	...
	navigationPrototype.renderRelatedGames = function(choices) {
		...
		for(var i = 0; i < choices.length; i++) {
			...
			faZepto('.fg-related-games').append('<li><a href="'+packageId+'/'+self.config.aid+'"><img src="'+thumb+'" alt=""></li>');
		}
	};
...
fg_api.prototype.getAppLink = function() {
	return '' + window.famobi_gameID + '.app/' + this.config.aid;
};
...
fg_api.prototype.modalModule = function() {
	...
	modalPrototype.closeAd = function() {
		M.closing = true;
...
fg_api.prototype.fullscreenModule = function() {
	...
	fullscreenPrototype.init = function() {
		...
		if (!self.config.aid.startsWith("A-MONKEY")) {
			...
			if (detection.is.ios || !self.hasFeature('fullscreen') || /^MacIntel|Win32$/.test(navigator.platform)) {
				this.isSupported = false;
			}
